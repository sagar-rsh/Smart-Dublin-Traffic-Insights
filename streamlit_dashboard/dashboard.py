...
# Database Connection
@st.cache_resource  # Reconnects every hour
def get_db_engine():
    try:
        engine = create_engine(
                    st.secrets["redshift"]["url"],
                    pool_size=3,
                    max_overflow=2,
                    pool_pre_ping=True,
                    pool_recycle=300,
...
@st.cache_data(ttl=600, show_spinner="Loading traffic data...")
def load_data():
    ...
    try:
        ...
        dim_tables = {}
        with engine.connect() as conn:
            for table in ["dim_routes", "dim_junctions", "dim_time"]:
                with st.spinner(f"Loading {table}..."):
                    dim_tables[table] = pd.read_sql(f"SELECT * FROM prod.{table}", con=conn.connection)
        
        return fact_trips, dim_tables["dim_routes"], dim_tables["dim_junctions"], dim_tables["dim_time"]
...
def process_data(fact_trips, dim_routes):
    try:
        # Convert datatypes
        fact_trips['time'] = pd.to_datetime(fact_trips['time'], format='%H:%M:%S', errors='coerce')
        fact_trips['date'] = pd.to_datetime(fact_trips['date'], errors='coerce')
        ...
        if not dim_routes.empty:
            fact_trips = fact_trips.merge(
                dim_routes[['route', 'link', 'direction_name']].drop_duplicates(),
                on='route',
                how='left'
            )
        ...
        for col in fact_trips.select_dtypes(include=['int64']):
            fact_trips[col] = pd.to_numeric(fact_trips[col], downcast='integer')
...
def safe_plotly_chart(fig):
    ...
    try:
        if len(fig.data[0].x) > 500000:
            st.image(fig.to_image(format="png", width=1200), use_column_width=True)
        else:
...
def main():
    ...
    cols[1].metric("Avg Travel Time", f"{filtered_df['avg_travel_time'].mean():.1f} sec")
    cols[2].metric("Peak Hour", f"{filtered_df.groupby('hour')['trip_count'].sum().idxmax()}:00")
    cols[3].metric("Busiest Route", f"Route {filtered_df['route'].value_counts().idxmax()}")
    ...
    with tab1:
        if not filtered_df.empty:
            ...
            avg_time_by_hour = filtered_df.groupby('hour')['avg_travel_time'].mean().reset_index()
            fig1 = px.line(
                avg_time_by_hour, 
                x='hour', 
                y='avg_travel_time', 
                ...
                labels={'avg_travel_time': 'Avg Travel Time (s)', 'hour': 'Hour of Day'}
            )
            safe_plotly_chart(fig1)
            
            # 2. Peak Hour Analysis
            st.subheader("Peak Hour Analysis")
            hourly_counts = filtered_df.groupby('hour')['trip_count'].sum().reset_index()
            fig2 = px.area(
                hourly_counts, 
                x='hour', 
                y='trip_count',
    ...
    with tab2:
        ...
        if 'junction_start' in filtered_df.columns and 'junction_end' in filtered_df.columns:
            junction_pairs = filtered_df.groupby(['junction_start', 'junction_end']).size().reset_index(name='count')
        ...
        if 'junction_start' in filtered_df.columns:
            top_starts = filtered_df['junction_start'].value_counts().head(10).reset_index()
        ...
        if 'junction_end' in filtered_df.columns:
            top_ends = filtered_df['junction_end'].value_counts().head(10).reset_index()
    ...
    with tab3:
        ...
        if not route_stats.empty:
            top_routes = route_stats.sort_values('avg_travel_time', ascending=False).head(10)
            fig4 = px.bar(
                top_routes, 
                x='route',
                y='avg_travel_time',
        ...
        if not filtered_df.empty:
            if route == 'All':
                ...
                top_routes = filtered_df['route'].value_counts().head(3).index.tolist()
                route_data = filtered_df[filtered_df['route'].isin(top_routes)]
                fig5 = px.line(
                    route_data.groupby(['date', 'route'])['avg_travel_time'].mean().reset_index(),
                    x='date', 
                    y='avg_travel_time', 
                    color='route',
                ...
                fig5 = px.line(
                    route_data.groupby('date')['avg_travel_time'].mean().reset_index(),
                    x='date', 
                    y='avg_travel_time',
            ...
            st.warning("No route performance data available with current filters")
